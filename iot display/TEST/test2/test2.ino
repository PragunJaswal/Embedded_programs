...
// int num =0;
// int address = 10;
// writeStringToEEPROM(address, num);
// delay(150);
// mess = readStringFromEEPROM(address);
// delay(150);
// if (mess%2 == 0){
// writeStringToEEPROM(address, num+1);
// ESP.restart();
// }
      ...
      case SHOW_YEAR:
        if (!dataFetched) {  // Fetch data only once when switching to SHOW_YEAR state
          if ((WiFi.status() == WL_CONNECTED)) {
            ...
            message = readStringFromEEPROM(addressToStore);
            Serial.println("Data from EEPROM: " + message);
          ...
          if ((WiFi.status() == WL_CONNECTED)) {
            ...
            if (int(toggle_state) == 0) {
              ...
              if (!topNews.isEmpty()) {
                ...
                writeStringToEEPROM(addressToStore, topNews);
                newsFetched = !newsFetched;
                scroll(topNews);
              } else {
                fail += 1;
              ...
              if (!notice.isEmpty()) {
                ...
                currentState = SCROLL_TEXT;
                writeStringToEEPROM(addressToStore, notice);
                newsFetched = !newsFetched;
                scroll(notice);
              } else {
                fail += 1;
